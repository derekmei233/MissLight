...
if __name__ == '__main__':
    """
    Currently we are conducting experiments on the HZ4x4 raw dataset. 
    rawstate file is generated by maxpressure agent interacting with simulation environment. 
    roadnet_relation file is generated by network structure.
    state_xx_astgcn is generated and masked by provided mask position and rawstate files
    state file is the final state used for training astgcn model
    Then the trained model is used as inference net to infer masked positions' traffic states
    Finally we have another round simulation, which uses the inference model to fill masked positions, 
    ...
    for i in test:
        ...
        # this is the experiment of  Maxpressure and FixedTime agents
        logger.info("Maxpressure&FixedTime")
        plan(0, masked_pos, tmp_action)
        logger.info('\n')
    # all agents are FixedTime agents.
